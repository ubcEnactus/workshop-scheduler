...
export default async function PALayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      <PASidebar signOutAction={signOutAction} />
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex size-8 items-center justify-center rounded-full bg-purple-600 text-xs font-bold text-white">
              {(user.name ?? user.email).slice(0, 2).toUpperCase()}
            </div>
            <div className="text-right">
              <p className="text-xs font-medium text-gray-900">{user.name ?? user.email}</p>
              <p className="text-[10px] text-gray-500 uppercase">PA</p>
            </div>
          </div>
        </header>
